...
//        }
        ...
        if (rGesture.getWidth() < SingleClick.SINGLE_CLICK_dLIMIT && rGesture.getHeight() < SingleClick.SINGLE_CLICK_dLIMIT) {
            if (rGesture.getDuration() <= SingleClick.SINGLE_CLICK_tLIMIT) {
                if (strokesNumber == 2) {
...
//        Looper.loop();
        ...
        for (Map.Entry<String,ArrayList<MediaDot>> node:
            ...
            for (MediaDot sDot:
                 sDotA) {
                recognizeGestures(new SimpleDot(sDot), strokesNumber);
            }
        }
...
//        }).start();
    ...
    public int processEachDot(MediaDot mediaDot){
        ...
        //完整坐标
        xD = simpleDot.getFloatX();
        yD = simpleDot.getFloatY();

        if (mediaDot.type == Dot.DotType.PEN_DOWN) {
            currentDotType = Dot.DotType.PEN_DOWN;
            Log.e(TAG, "PEN_DOWN");
            bufferInit(simpleDot.getFloatX(), simpleDot.getFloatY());//初始化暂存区
...
//            simpleDots.add(simpleDot);
        ...
        rectF.union(simpleDot.getFloatX(), simpleDot.getFloatY());
//        processRect();
        ...
        if (mediaDot.type == Dot.DotType.PEN_MOVE) {
            ...
            if (rectF.width() > SingleClick.SINGLE_CLICK_dLIMIT || rectF.height() > SingleClick.SINGLE_CLICK_dLIMIT) {
...
//                simpleDots.add(simpleDot);
            ...
            } else if ((mediaDot.timelong - strokeBeginT) < LongPress.LONG_PRESS_tLIMIT && (mediaDot.timelong - strokeBeginT) > SingleClick.SINGLE_CLICK_tLIMIT) {
                //不是单击、双击，可能是长压
                mediaDots.add(mediaDot);
//                simpleDots.add(simpleDot);
                return -1;
