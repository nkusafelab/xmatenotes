...
//        }
        ...
        if(mediaDot.type == Dot.DotType.PEN_DOWN){
            ...
            if(mediaDot.isAudioDot() && mediaDot.audioID != lastAudioID){
                ...
                page.addAudio(mediaDot.getIntX(), mediaDot.getIntY(), Page.createRecordAudioName(mediaDot.audioID));
            }
            Log.e(TAG,"if(page != lastPage)");

            LocalRect lR = excelReader.getLocalRectByXY(page.getPageNumber(), mediaDot.getIntX(), mediaDot.getIntY());
            if(page != lastPage){
                //跨页当前页已存储底图数量自动+1
