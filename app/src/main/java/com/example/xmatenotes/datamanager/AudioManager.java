...
public class AudioManager {
    ...
    private volatile static AudioManager audioManager;
    ...
    public void audioInit(Activity activity){
        ...
        if(Build.VERSION.SDK_INT >=23){
            ...
            for(int i=0;i<permission.length;i++){
                //判断是否有权限
                if(XApp.context.checkSelfPermission(permission[i]) != PackageManager.PERMISSION_GRANTED){
    ...
    public void startPlayAssetsAudio(String audioFileName) throws IOException {
        ...
        AssetFileDescriptor afd = XApp.context.getApplicationContext().getAssets().openFd("audio/"+audioFileName);
        if(player == null){
...
//                comPlayAssetsAudio("endrecord.mp3");
     ...
     */
    @SneakyThrows
    public void comPlayAssetsAudio(String audioFileName) {
        startPlayAssetsAudio(audioFileName);
        while (isPlaying()){
